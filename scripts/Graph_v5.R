...
Data <- merge(Demo[c("bblid", "scanid", "sex", "ageAtScan1","age2",
  "handednessv2")], TQa[c("scanid", "averageManualRating", "t1Exclude")],
  by = "scanid")
...
Data[,!(names(Data) %in% c("scanid", "bblid", "sex", "handednessv2", "group",
  "pcaslExclude", "t1Exclude"))] <- scale(Data[,!(names(Data) %in% c("scanid",
  "bblid", "sex", "handednessv2", "group", "pcaslExclude", "t1Exclude"))],
  ...
  for(y in c("F1", "F2", "F3", "M1", "M2", "M3")){
    for(z in names$full){
      Final[w,"effect"] <- lm(as.formula(paste0(z,
        "_", x, " ~ ageAtScan1 + age2 + handednessv2 + ", qa)),
